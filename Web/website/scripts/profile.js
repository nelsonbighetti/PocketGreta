...
async function loginCreds(email, password, userdriven){
    ...
    await axios.post('http://localhost:5000/rest/auth/login', params
    ).catch(error => {
        if(userdriven){
...
async function doregister() {
    ...
    await axios.post('http://localhost:5000/rest/auth/signup', params
    ).catch(error => {
...
function refreshToken(){
    ...
    axios.post('http://localhost:5000/rest/refreshToken', params
    ).catch(error => {
        document.getElementById("register_error").style.visibility = "visible"
    }).then(response => {
        if(response.status!==200){
...
async function checkLogin() {
    ...
    if(username){
        ...
        await axios.post('http://localhost:5000/rest/acc/', params
        ).catch(error => {
            document.getElementById("login_error").style.visibility = "visible"
        }).then(response => {
            if(response.status!==200){
